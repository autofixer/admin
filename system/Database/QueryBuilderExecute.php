...
trait QueryBuilderExecute
{
    ...
    public function go() : PDOStatement
    {
        ...
        // not failed
        if ($instance->failed === false) :

            // call Prefix Query method
            $instance->callPrefixQuery($this);

            // query not null
            if ($instance->returnNewQuery !== null) :

                // return a new query
                return $instance->returnNewQuery;

            endif;

            // hash name
            $hashname = md5($instance->query . implode('', $instance->bind));
            
            // not cached
            if (!isset(self::$executeQueryCache[$hashname])) :
                
                // process request.
                if ($this->__checkForErrors($instance->method) === true) :
                    ...
                    if ($instance->method == 'select') :
                        ...
                        foreach ($this->bind as $key => $val) :
                            ...
                            if (is_null($val) || (is_string($val) && strlen($val) == 0)) :
                                ...
                                foreach ($instance->bind as $value) :
                    ...
                    $instance->query = str_replace('{where}', '', $instance->query);

                    // remove placeholder {-where-}
                    $instance->query = str_replace('{-where-}', '', $instance->query);

                    
                    if (!$instance->allowHTMLTags) :
                    ...
                    // cache
                    self::$executeQueryCache[$hashname] = [$instance->query, $instance->bind];
                    
                endif;

            else:

                list($instance->query, $instance->bind) = self::$executeQueryCache[$hashname];
            ...
            if ($instance->method != '') : 
                ...
                try
                {
                    ...
                    if ($instance->allowQuery()) :

                        // prepare statement
                        $statement = $this->___prepare($instance->query);
                        ...
                        if ($statement->execute()) :
                            ...
                            switch ($instance->method) :
                                ...
                                case 'update':
                                case 'delete':
                                    if (!defined('DB_TEST_ENV')) $this->saveQueryStatement($instance->query, $instance->bind);
                                    $instance->queryCachePath = null;
                                break;
        
                            endswitch;

                            // log last insert id
                            if ($instance->method == 'insert') Handler::$lastInsertId = $instance->pdoInstance->lastInsertId();

                            // query ran
                            Handler::queryRanSuccessfully($statement, $instance->method);
                            ...
                            // commit transaction
                            if (method_exists($instance->pdoInstance, 'commit') && $instance->method == 'select') :
                            
                                if ($instance->pdoInstance->inTransaction()) : $instance->pdoInstance->commit(); endif;

                            endif;

                            // load subscribers
                            if ($instance->method != 'select') Handler::loadSubscribers($statement, $instance->pdoInstance);
                ...
                {
                    if (method_exists($instance->pdoInstance, 'inTransaction') && $instance->pdoInstance->inTransaction() === true) :

                        // rollback transaction
                        $instance->pdoInstance->rollBack();
