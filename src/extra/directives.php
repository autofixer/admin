...
Directives::register([
     ...
     * Would check for alert
     */
    'isloggedin' => function() : string 
    {
        return '<?php if (session()->has("auth.user")) { $user = session()->get("auth.user"); ?>';
    },
    ...
    {
        // @var string $target
        $target = func()->finder('beautiful_url_target');
    ...
    'currentView' => function() : string 
    {
        ...
        // get route
        $route = explode('/', (isset($_GET[$target]) ? $_GET[$target] : ''));

        // return string
        return ucwords($route[0] == 'home' ? 'dashboard' : $route[0]);
    },
     ...
     * @return string 
     */
    'isView' => function(string $view) : string 
    {
        // mode 
        $mode = '';
        ...
        // check now
        if (implode('/', $route) == $view) $mode = 'active';
        if (isset($route[0]) && $mode == '' && $route[0] == $view) $mode = 'active';
